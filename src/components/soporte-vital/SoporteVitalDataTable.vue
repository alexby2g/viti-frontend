...
<template>
  <q-table flat class="sv-table" :rows="rows" :columns="columns" row-key="id" :grid="grid" :pagination="{rowsPerPage:20}">
    ...
    <template #body-cell-actions="p"><q-td :props="p"><q-btn flat round dense color="primary" icon="visibility" @click="emit('view',p.row)"/><q-btn v-if="!locked(p.row)" flat round dense icon="edit" @click="emit('edit',p.row)"/><q-btn v-if="!locked(p.row)" flat round dense color="negative" icon="delete" @click="emit('delete',p.row)"/></q-td></template>
