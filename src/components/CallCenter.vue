...
async function openMedia(type) {
  ...
  const constraints = {
    ...
    video: type === 'video' ? { facingMode: 'user', width: { ideal: 1280 }, height: { ideal: 720 }, frameRate: { ideal: 24, max: 30 } } : false,
  }
  ...
  localStream.getTracks().forEach(track => {
    track.enabled = true
    track.onended = () => { if (track.kind === 'video') { localVideoReady.value = false; mediaHint.value = 'La cámara se detuvo. Revisa los permisos o si otra aplicación está usando la cámara.' } }
  })
  markLocalState()
  await attachStreams()
}
...
function setupDataChannel(channel) {
  ...
  dataChannel.onmessage = event => {
    try {
      ...
      if (payload.type === 'screen-frame' && payload.data) { remoteScreenFrame.value = payload.data; mediaHint.value = '' }
...
function createPeer(isInitiator = false) {
  ...
  peer.onconnectionstatechange = () => {
    if (peer?.connectionState === 'connected') { phase.value = 'active'; watchRemoteVideo() }
...
async function startOutgoing(detail) {
  ...
  try {
    photoFailed.value = false; phase.value = 'calling'; await openMedia(detail.type); createPeer(true)
...
async function answerIncoming() {
  ...
  try {
    photoFailed.value = false; phase.value = 'connecting'; mediaType.value = item.tipo; call.value = item; incoming.value = null
...
function sendScreenPayload(payload) { if (dataChannel?.readyState !== 'open') return false; try { dataChannel.send(JSON.stringify(payload)); return true } catch { return false } }
...
async function startScreenShare() {
  ...
  if (window.Android?.startVitiScreenShare) {
    ...
    $q.notify({ type:'info', message:'Compartiendo VITI. La llamada seguirá activa mientras navegas por la aplicación.' })
    return
  }
  if (!navigator.mediaDevices?.getDisplayMedia) return notifyError('Este dispositivo necesita la versión Android de VITI con asistencia de pantalla.')
  try {
...
function handleAndroidScreenFrame(event) { if (!screenSharing.value || !event?.detail?.data) return; sendScreenPayload({ type:'screen-frame', data:event.detail.data }) }
function minimizeCall() { if (active.value) minimized.value = true }
...
function cleanup(clearIncoming = true) {
  ...
  phase.value = 'idle'; mediaType.value = 'audio'; muted.value = false; cameraOff.value = false; photoFailed.value = false; localVideoReady.value = false; remoteVideoReady.value = false; mediaHint.value = ''; pendingCandidates = []; pendingRemoteCandidates = []; lastSignalId = 0
}
  ...
  <q-dialog :model-value="active && !minimized" persistent maximized transition-show="slide-up" transition-hide="slide-down">
    <q-card class="call-screen column no-wrap">
      <q-card-section class="row items-center call-header">
        ...
        <div><div class="text-h6 text-weight-bold">{{ otherPerson }}</div><div class="text-caption">{{ statusLabel }} · {{ supportMode === 'pantalla' ? 'Soporte de pantalla' : (isVideo ? 'Video' : 'Audio') }}</div></div>
        <q-space />
        <q-btn round flat color="white" icon="remove" @click="minimizeCall"><q-tooltip>Minimizar llamada</q-tooltip></q-btn>
      ...
      <q-card-section class="call-controls row justify-center q-gutter-md">
        ...
        <q-btn v-if="canShareScreen" round size="lg" :color="screenSharing ? 'primary' : 'grey-8'" :icon="screenSharing ? 'stop_screen_share' : 'screen_share'" @click="screenSharing ? stopScreenShare() : startScreenShare()" />
        <q-btn round size="lg" color="negative" icon="call_end" @click="finish('finalizada')" />
      </q-card-section>
    </q-card>
  </q-dialog>

  <div v-if="active && minimized" class="call-mini">
    <q-avatar size="38px" color="primary" text-color="white"><img v-if="otherPhoto" :src="otherPhoto" /><q-icon v-else name="support_agent" /></q-avatar>
    ...
    <q-btn round flat color="white" icon="open_in_full" @click="restoreCall"><q-tooltip>Volver a la llamada</q-tooltip></q-btn>
    <q-btn round color="negative" icon="call_end" @click="finish('finalizada')" />
  </div>
</template>

<style scoped>
.call-incoming-card{width:380px;max-width:92vw;border-radius:24px}.call-screen{background:#050b13;color:#fff}.call-header{background:#0b2447}.call-stage{position:relative;display:flex;align-items:center;justify-content:center;overflow:hidden;min-height:0;background:#000}.remote-video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;background:#000}.remote-video.video-small{left:auto;right:18px;bottom:18px;top:auto;width:min(24vw,190px);height:min(32vw,250px);border-radius:16px;border:2px solid rgba(255,255,255,.55);z-index:4}.remote-screen{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;background:#050505;z-index:2}.local-video-wrap{position:absolute;right:18px;top:18px;width:min(28vw,210px);height:min(38vw,280px);border-radius:18px;border:2px solid rgba(255,255,255,.5);background:#111;overflow:hidden;z-index:5}.local-video{width:100%;height:100%;object-fit:cover;background:#111}.local-video-fallback{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:#111;color:#fff}.video-waiting{position:relative;z-index:2;text-align:center;color:#fff;padding:24px;max-width:520px}.audio-avatar{text-align:center}.media-hint{background:#14233a;color:#dce7f7;padding:10px 18px;text-align:center;font-size:13px}.call-controls{background:#0b2447;padding-bottom:max(20px,env(safe-area-inset-bottom));z-index:6}.call-mini{position:fixed;left:50%;bottom:max(18px,env(safe-area-inset-bottom));transform:translateX(-50%);width:min(520px,92vw);z-index:9999;display:flex;align-items:center;gap:10px;background:#0b2447;color:#fff;padding:9px 11px;border-radius:18px;box-shadow:0 10px 30px rgba(0,0,0,.35)}
</style>
