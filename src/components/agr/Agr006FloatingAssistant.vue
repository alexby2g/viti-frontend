<template>
  <div
    v-if="visible"
    class="agr006-root"
    :class="[`state-${state}`, { open, 'voice-armed': voiceArmed, 'voice-active': voiceActive, speaking: speaking }]"
  >
    <div v-if="voiceActive" class="agr006-hologram-shell" aria-hidden="true">
      ...
      <svg class="agr006-hologram" viewBox="0 0 420 720" role="img" aria-label="006 holograma">
        <defs>
          <linearGradient id="holoStroke" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0" stop-color="#dffbff" stop-opacity=".95"/>
            <stop offset=".35" stop-color="#63eaff" stop-opacity=".85"/>
            <stop offset="1" stop-color="#168dff" stop-opacity=".25"/>
          </linearGradient>
          <radialGradient id="holoFill" cx="50%" cy="40%" r="70%">
            <stop offset="0" stop-color="#9ef7ff" stop-opacity=".28"/>
            <stop offset=".55" stop-color="#167dff" stop-opacity=".12"/>
        ...
        <g class="holo-head" filter="url(#glow)">
          ...
          <path class="mouth" d="M194 224 Q210 232 226 224"/>
      ...
      <div class="holo-wave" :class="{ active: speaking || voiceActive }"><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div>
    </div>

    <button v-else class="agr006-orb" type="button" :aria-label="open ? 'Cerrar 006' : 'Abrir 006'" @click="toggle">
    ...
    <section v-if="open" class="agr006-panel" aria-label="Asistente 006">
      ...
      <div class="agr006-body">
        ...
        <div v-if="voiceActive" class="agr006-transcription"><span class="agr006-live-dot"></span><span>{{ text || 'Escuchando…' }}</span></div>
        ...
        <form class="agr006-input" @submit.prevent="sendText">
          ...
          <button type="button" :class="['voice-btn', { listening }]" @click="toggleVoice" :aria-label="listening ? 'Detener escucha' : 'Activar escucha'">{{ listening ? '■' : '●' }}</button>
          <button type="submit" :disabled="!text.trim()">→</button>
        </form>
        <small class="agr006-hint">{{ voiceActive ? 'Di “desactivar VITI” para terminar.' : 'Activa el micrófono una vez y di “activar VITI”.' }}</small>
      </div>
    </section>
  </div>
</template>
...
function toggle() {
  ...
  if (open.value && messages.value.length === 0) messages.value.push({ role: 'assistant', text: '006 en línea. VITI está operativo. ¿Qué necesitas?' })
}
function extractPayload(response: any) { return response?.data || {} }
function extractAnswer(payload: any) { return payload?.data?.message ?? payload?.message ?? payload?.data?.data?.message ?? 'No tengo una respuesta disponible todavía.' }
function extractAction(payload: any): AgrAction | null { return payload?.data?.action ?? payload?.action ?? payload?.data?.data?.action ?? null }
...
async function ask(message: string) {
  ...
  try {
    ...
    messages.value.push({ role: 'assistant', text: status === 401 || status === 419 ? 'La sesión de VITI no está lista. Mantén abierta la sesión e inténtalo de nuevo.' : 'No pude consultar VITI en este momento. Revisa la conexión del sistema.' })
  }
}
...
async function executeAction(action: AgrAction | null | undefined) {
  ...
  else if (action.type === 'browser_back') await router.back()
}

function sendText() { const value = text.value.trim(); if (!value) return; text.value = ''; void ask(value) }
function runCommand(command: string) { void ask(command) }

function normalizeVoiceText(value: string) { return value.toLocaleLowerCase('es-BO').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[¡!¿?.,;:]/g, ' ').replace(/\s+/g, ' ').trim() }
function hasWakeWord(value: string) { return /(activar viti|activar 006|activar v i t i)/.test(normalizeVoiceText(value)) }
function hasSleepWord(value: string) { return /(desactivar viti|desactiva viti|detener viti|silencio viti)/.test(normalizeVoiceText(value)) }
function cleanWakePhrase(value: string) { return value.replace(/activar\s+(viti|006|v\s*i\s*t\s*i)/i, '').trim() }
...
function startRecognitionLoop() {
  ...
  recognition.onend = () => { listening.value = false; if (voiceArmed.value) { if (restartTimer) clearTimeout(restartTimer); restartTimer = setTimeout(() => startRecognitionLoop(), 300) } }
  recognition.onerror = () => { listening.value = false; if (voiceArmed.value) { if (restartTimer) clearTimeout(restartTimer); restartTimer = setTimeout(() => startRecognitionLoop(), 700) } }
  recognition.onresult = (event: any) => {
    ...
    if (hasSleepWord(combined)) { deactivateVoiceMode(); return }
    if (finalText) {
...
function toggleVoice() {
  if (!(('webkitSpeechRecognition' in window) || ('SpeechRecognition' in window))) { state.value = 'attention'; messages.value.push({ role: 'assistant', text: 'Este navegador no expone reconocimiento de voz. Puedes escribirme.' }); return }
  if (voiceArmed.value) { deactivateVoiceMode(); return }
...
async function speakWith006(textToSpeak: string) {
  ...
  try {
    ...
    activeAudio.onended = () => { speaking.value = false; stopAudio() }
  ...
  fallback.rate = 0.90
  fallback.pitch = 0.78
  fallback.onend = () => { speaking.value = false }
...
<style scoped>
.agr006-root{position:fixed;right:24px;bottom:24px;z-index:9999;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#eaf7ff}.agr006-orb{position:relative;width:78px;height:78px;border:0;border-radius:50%;background:radial-gradient(circle at 50% 46%,rgba(255,255,255,.95),rgba(117,225,255,.55) 17%,rgba(37,150,255,.2) 38%,rgba(0,0,0,0) 67%);box-shadow:0 0 22px rgba(53,200,255,.55),0 0 70px rgba(53,200,255,.2);cursor:pointer;animation:float 3.8s ease-in-out infinite}.agr006-core{position:absolute;inset:18px;border-radius:50%;display:grid;place-items:center;background:rgba(7,25,41,.72);border:1px solid rgba(165,237,255,.7);box-shadow:inset 0 0 18px rgba(72,211,255,.45),0 0 18px rgba(72,211,255,.35);font-weight:800;letter-spacing:.14em;font-size:12px}.agr006-ring{position:absolute;inset:5px;border:1px solid rgba(126,231,255,.55);border-radius:50%;transform:rotate(22deg)}.ring-a{animation:spin 7s linear infinite}.ring-b{inset:10px;border-style:dashed;opacity:.55;animation:spinReverse 11s linear infinite}.agr006-status-dot{position:absolute;right:7px;bottom:8px;width:10px;height:10px;border-radius:50%;background:#7dffb2;box-shadow:0 0 10px #7dffb2}.voice-armed .agr006-status-dot{background:#ffe680;box-shadow:0 0 12px #ffe680;animation:pulse .9s infinite}.voice-active .agr006-status-dot{background:#63f7ff;box-shadow:0 0 18px #63f7ff;animation:pulse .55s infinite}
.agr006-hologram-shell{position:fixed;right:48px;bottom:38px;width:min(440px,42vw);height:min(720px,80vh);display:flex;align-items:flex-end;justify-content:center;pointer-events:none;filter:drop-shadow(0 0 24px rgba(53,200,255,.45));animation:holoAppear .7s ease-out both}.agr006-hologram{position:relative;width:100%;height:100%;overflow:visible}.holo-aura{position:absolute;inset:14% 12% 3%;background:radial-gradient(ellipse at center,rgba(61,210,255,.18),rgba(20,120,255,.06) 48%,transparent 72%);filter:blur(16px);animation:aura 2.8s ease-in-out infinite}.holo-grid{position:absolute;left:10%;right:10%;bottom:7%;height:30%;opacity:.18;background:repeating-linear-gradient(0deg,rgba(100,230,255,.7) 0 1px,transparent 1px 16px),repeating-linear-gradient(90deg,rgba(100,230,255,.4) 0 1px,transparent 1px 22px);mask-image:linear-gradient(to top,black,transparent);animation:gridMove 4s linear infinite}.holo-body{transform-origin:center bottom;animation:bodyFloat 3.4s ease-in-out infinite}.body-fill,.head-fill{fill:url(#holoFill)}.body-stroke,.head-stroke,.lapel,.tie,.arm,.hand,.leg,.hair,.ear,.eye,.nose,.mouth,.jaw,.forehead-mark,.holo-base,.holo-scan{fill:none;stroke:url(#holoStroke);stroke-width:3;stroke-linecap:round;stroke-linejoin:round}.holo-head{transform-origin:210px 170px;animation:headFloat 5s ease-in-out infinite}.hair{stroke-width:5}.forehead-mark{fill:#dffbff;filter:drop-shadow(0 0 9px #53eaff)}.eye{fill:#b8fbff;stroke-width:2;animation:blink 5.5s ease-in-out infinite}.mouth{stroke-width:3}.speaking .mouth{animation:talk 150ms ease-in-out infinite alternate}.speaking .holo-head{animation:talkHead 1.6s ease-in-out infinite}.speaking .left-hand{animation:gestureLeft 1.5s ease-in-out infinite alternate;transform-origin:110px 462px}.speaking .right-hand{animation:gestureRight 1.9s ease-in-out infinite alternate;transform-origin:310px 462px}.voice-active:not(.speaking) .left-hand{animation:listeningGesture 2.8s ease-in-out infinite alternate;transform-origin:110px 462px}.holo-base{stroke-width:4;opacity:.9}.holo-scan{stroke-width:2;opacity:.4;animation:scan 2.2s linear infinite}.holo-caption{position:absolute;left:50%;bottom:8%;transform:translateX(-50%);display:flex;gap:8px;align-items:center;padding:8px 14px;border-radius:999px;border:1px solid rgba(108,232,255,.28);background:rgba(4,20,34,.75);backdrop-filter:blur(10px);box-shadow:0 0 25px rgba(50,200,255,.16);white-space:nowrap}.holo-caption strong{font-size:13px;letter-spacing:.22em}.holo-caption span:last-child{font-size:10px;color:#8fe8ff}.status-dot{width:7px;height:7px;border-radius:50%;background:#6bffb0;box-shadow:0 0 12px #6bffb0}.speaking .status-dot{background:#ffd86b;box-shadow:0 0 12px #ffd86b}.holo-wave{position:absolute;left:50%;bottom:3%;transform:translateX(-50%);display:flex;gap:3px;height:24px;align-items:center;opacity:.65}.holo-wave i{display:block;width:3px;height:8px;border-radius:3px;background:#63edff}.holo-wave.active i{animation:wave .6s ease-in-out infinite alternate}.holo-wave i:nth-child(2){animation-delay:.08s}.holo-wave i:nth-child(3){animation-delay:.16s}.holo-wave i:nth-child(4){animation-delay:.24s}.holo-wave i:nth-child(5){animation-delay:.32s}.holo-wave i:nth-child(6){animation-delay:.4s}.holo-wave i:nth-child(7){animation-delay:.48s}
.agr006-panel{position:absolute;right:0;bottom:92px;width:min(420px,calc(100vw - 32px));max-height:560px;overflow:hidden;border:1px solid rgba(130,220,255,.2);border-radius:20px;background:linear-gradient(180deg,rgba(8,23,37,.96),rgba(3,12,22,.97));backdrop-filter:blur(18px);box-shadow:0 20px 70px rgba(0,0,0,.45),0 0 50px rgba(53,200,255,.12)}.agr006-header{display:flex;justify-content:space-between;align-items:center;padding:16px 18px;border-bottom:1px solid rgba(255,255,255,.07)}.agr006-header strong{display:block;font-size:18px;letter-spacing:.18em}.agr006-header small{display:block;margin-top:2px;color:#86a8b8;font-size:11px}.agr006-close{background:transparent;border:0;color:#92aebd;font-size:24px;cursor:pointer}.agr006-body{padding:16px}.agr006-presence{margin:0 0 12px;color:#a9cedd;font-size:13px}.agr006-messages{display:flex;flex-direction:column;gap:9px;max-height:320px;overflow:auto;margin-bottom:12px}.agr006-message{max-width:88%;padding:10px 12px;border-radius:14px;font-size:13px;line-height:1.4}.agr006-message.user{align-self:flex-end;background:rgba(58,157,214,.17);border:1px solid rgba(98,197,244,.15)}.agr006-message.assistant{align-self:flex-start;background:rgba(255,255,255,.045);border:1px solid rgba(255,255,255,.07)}.agr006-transcription{display:flex;gap:8px;align-items:center;margin-bottom:10px;padding:10px 12px;border-radius:12px;background:rgba(63,229,255,.08);border:1px solid rgba(63,229,255,.14);color:#b8f5ff;font-size:12px}.agr006-live-dot{width:7px;height:7px;border-radius:50%;background:#5ff4ff;box-shadow:0 0 12px #5ff4ff;animation:pulse .7s infinite}.agr006-actions{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 12px}.agr006-actions button{border:1px solid rgba(133,220,255,.18);border-radius:10px;background:rgba(77,180,239,.1);color:#dff7ff;padding:7px 10px;cursor:pointer;font-size:12px}.agr006-input{display:flex;gap:8px}.agr006-input input{min-width:0;flex:1;border:1px solid rgba(133,220,255,.18);border-radius:12px;background:rgba(255,255,255,.035);color:#fff;padding:11px 12px;outline:none}.agr006-input button{width:40px;border:1px solid rgba(133,220,255,.18);border-radius:12px;background:rgba(77,180,239,.14);color:#dff7ff;cursor:pointer}.voice-btn.listening{background:rgba(255,92,92,.2);border-color:rgba(255,112,112,.4)}.agr006-hint{display:block;margin-top:8px;color:#7094a6;font-size:10px;text-align:center}
@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-7px)}}@keyframes spin{to{transform:rotate(382deg)}}@keyframes spinReverse{to{transform:rotate(-382deg)}}@keyframes pulse{50%{opacity:.35}}@keyframes holoAppear{from{opacity:0;transform:translateY(20px) scale(.96)}to{opacity:1;transform:none}}@keyframes aura{50%{transform:scale(1.05);opacity:.9}}@keyframes gridMove{to{background-position:0 40px,26px 0}}@keyframes bodyFloat{50%{transform:translateY(-7px)}}@keyframes headFloat{50%{transform:translate(1px,-3px) rotate(.8deg)}}@keyframes blink{0%,44%,48%,100%{opacity:1}46%{opacity:.08}}@keyframes talk{from{transform:scaleY(.35)}to{transform:scaleY(1.25)}}@keyframes talkHead{50%{transform:translateY(-2px) rotate(-.6deg)}}@keyframes gestureLeft{from{transform:rotate(-2deg)}to{transform:rotate(4deg) translate(2px,-3px)}}@keyframes gestureRight{from{transform:rotate(2deg)}to{transform:rotate(-3deg) translate(-1px,2px)}}@keyframes listeningGesture{from{transform:rotate(-1deg)}to{transform:rotate(2deg) translateY(-2px)}}@keyframes scan{from{transform:translateX(-25px)}to{transform:translateX(25px)}}@keyframes wave{from{height:5px}to{height:22px}}
@media (max-width:900px){.agr006-hologram-shell{right:10px;bottom:30px;width:min(380px,65vw);height:70vh}.agr006-panel{right:0;bottom:88px}.agr006-hologram-shell+.agr006-panel{display:none}}@media (max-width:640px){.agr006-root{right:14px;bottom:14px}.agr006-hologram-shell{right:0;bottom:55px;width:90vw;height:68vh;opacity:.78}.agr006-orb{width:68px;height:68px}}
</style>