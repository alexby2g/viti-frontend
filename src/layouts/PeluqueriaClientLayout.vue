...
async function loadState(){loading.value=true;try{appInfo.value=(await api.get('/mi/apps/peluqueria/estado')).data.data}catch(e){$q.notify({type:'negative',message:e.response?.data?.message||'No se pudo abrir Peluquería.'});sessionStorage.setItem('viti-app-explicit-exit','1');router.replace('/mi-proyecto')}finally{loading.value=false}}
