...
const menu=computed(()=>{
  ...
  if(isSuperAdmin.value){
    operational.splice(1,0,{label:'Plataforma',icon:'hub',children:[{label:'Centro SaaS VITI',icon:'cloud_circle',to:'/saas'}]})
    operational.push(
...
onMounted(async()=>{document.body.classList.add('viti-main-active');drawer.value=$q.screen.gt.sm;if(isClient.value){try{await tenant.load()}catch{/* mantiene acceso básico */}}notifications.start();document.addEventListener('visibilitychange',refreshWhenVisible)})
onBeforeUnmount(()=>{document.body.classList.remove('viti-main-active');notifications.stop();document.removeEventListener('visibilitychange',refreshWhenVisible)})
</script>
