...
const adminPhotoInput=ref(null),uploadingPhoto=ref(false),router=useRouter(),$q=useQuasar(),drawer=ref(false),quickSearch=ref(false),quickTerm=ref('')
...
const accountRoleLabel=computed(()=>isClient.value?(tenant.active?.rol||'Cliente'):(isSuperAdmin.value?'Superadministrador':auth.user?.rol==='administrador'?'Administrador':auth.user?.rol||'Usuario interno'))
...
const menu=computed(()=>{
  if(isClient.value){
    ...
    items.push({label:'Mi negocio',icon:'storefront',to:'/mi-negocio'},{label:'Aplicaciones',icon:'apps',children:[{label:'Instaladas',icon:'grid_view',to:'/mi-aplicaciones'},...(isManager.value?[{label:'Catálogo VITI',icon:'widgets',to:'/catalogo-viti'}]:[])]})
    if(isManager.value)items.push({label:'Mi proyecto',icon:'account_tree',children:[{label:'Avances y archivos',icon:'timeline',to:'/mi-proyecto'},{label:'Pagos',icon:'payments',to:'/mi-pagos'}]})
  ...
  const operational = [
    ...
    {label:'Desarrollo',icon:'terminal',children:[{label:'Solicitudes',icon:'fact_check',to:'/solicitudes'},{label:'Proyectos',icon:'account_tree',to:'/proyectos'},{label:'Aplicaciones',icon:'apps',to:'/aplicaciones'}]},
    {label:'Atención',icon:'forum',children:[{label:'Mensajes de empresas',icon:'mark_chat_unread',to:'/buzon',badge:notifications.unreadCount},{label:'Casos de soporte',icon:'build_circle',to:'/mantenimientos'}]},
  ...
  if(isSuperAdmin.value){
    operational.splice(1,0,{label:'Planes y cobros',icon:'hub',children:[{label:'Planes y módulos',icon:'cloud_circle',to:'/saas'},{label:'Pagos VITI',icon:'payments',to:'/pagos'}]})
    const files=operational.find(item=>item.label==='Archivos');if(files)files.children.push({label:'Almacenamiento técnico',icon:'cloud',to:'/almacenamiento'})
    operational.push(
      {label:'Administración',icon:'admin_panel_settings',children:[{label:'Usuarios',icon:'manage_accounts',to:'/usuarios'},{label:'Auditoría',icon:'history',to:'/auditoria'}]},
    )
  }
  return operational
})
const quickItems=computed(()=>menu.value.flatMap(item=>item.children?item.children.map(child=>({...child,group:item.label})):item.to?[{...item,group:null}]:[]))
const filteredQuickItems=computed(()=>{const term=quickTerm.value.trim().toLocaleLowerCase('es');return (term?quickItems.value.filter(item=>`${item.group||''} ${item.label}`.toLocaleLowerCase('es').includes(term)):quickItems.value).slice(0,12)})
