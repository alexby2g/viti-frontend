...
function openBusiness360(company){ if (company?.id) router.push({name:'empresa-360',params:{id:company.id}}) }
async function save(){
  try{
    if(editing.value){
...
<q-page class="viti-page">
  ...
  <q-table v-if="$q.screen.gt.sm" flat class="viti-table" :rows="rows" :columns="columns" row-key="id" :loading="loading" :pagination="{rowsPerPage:20,sortBy:'id',descending:true}">
    ...
    <template #body-cell-actions="p"><q-td :props="p"><q-btn flat round dense icon="more_vert"><q-menu><q-list style="min-width:230px"><q-item clickable v-close-popup @click="openEdit(p.row)"><q-item-section avatar><q-icon name="edit"/></q-item-section><q-item-section>Editar responsable</q-item-section></q-item><q-item v-if="p.row.foto_path" clickable v-close-popup @click="verifyPhoto(p.row)"><q-item-section avatar><q-icon :name="p.row.foto_verificada?'verified':'verified_user'"/></q-item-section><q-item-section>{{p.row.foto_verificada?'Quitar verificación de foto':'Verificar fotografía'}}</q-item-section></q-item><q-item clickable v-close-popup @click="openCompanies(p.row)"><q-item-section avatar><q-icon name="business"/></q-item-section><q-item-section>Empresas asociadas</q-item-section></q-item><q-item v-if="p.row.empresas?.length" clickable v-close-popup @click="openBusiness360(p.row.empresas[0])"><q-item-section avatar><q-icon name="space_dashboard"/></q-item-section><q-item-section>Vista empresarial 360</q-item-section></q-item><q-separator/><q-item clickable v-close-popup class="text-negative" @click="remove(p.row)"><q-item-section avatar><q-icon name="delete_outline"/></q-item-section><q-item-section>Eliminar</q-item-section></q-item></q-list></q-menu></q-btn></q-td></template>
    <template #body-cell-empresas="p"><q-td :props="p"><div v-if="p.row.empresas?.length" class="column q-gutter-xs"><div v-for="e in p.row.empresas.slice(0,2)" :key="e.id"><q-badge outline color="primary" class="cursor-pointer" @click="openBusiness360(e)">{{e.nombre_comercial}}</q-badge></div><span v-if="p.row.empresas.length>2" class="text-caption text-grey-6">+{{p.row.empresas.length-2}} más</span></div><q-btn v-else flat dense no-caps color="primary" icon="add_business" label="Agregar empresa" @click="openCompanies(p.row)"/></q-td></template>
  ...
  <div v-else class="mobile-data-list">
    ...
    <q-card v-for="row in rows" :key="row.id" flat class="viti-card mobile-data-card">
      <q-card-section class="row items-start no-wrap">
        ...
        <div class="col q-ml-md min-width-0"><div class="text-subtitle1 text-weight-bold ellipsis">{{row.nombre}}</div><div class="text-caption text-grey-6">Responsable · {{row.telefono}}</div><div v-if="row.correo" class="text-caption text-grey-6">{{row.correo}}</div><div class="q-mt-sm"><q-badge v-for="e in row.empresas?.slice(0,2)" :key="e.id" outline color="primary" class="q-mr-xs q-mb-xs cursor-pointer" @click="openBusiness360(e)">{{e.nombre_comercial}}</q-badge><span v-if="!row.empresas?.length" class="text-caption text-grey-6">Sin empresa registrada</span></div><div class="text-caption text-grey-6 q-mt-xs">{{row.solicitudes_count||0}} solicitudes · {{row.estado}}</div></div>
        <q-btn flat round dense icon="more_vert"><q-menu><q-list style="min-width:230px"><q-item clickable v-close-popup @click="openEdit(row)"><q-item-section avatar><q-icon name="edit"/></q-item-section><q-item-section>Editar responsable</q-item-section></q-item><q-item clickable v-close-popup @click="openCompanies(row)"><q-item-section avatar><q-icon name="business"/></q-item-section><q-item-section>Empresas asociadas</q-item-section></q-item><q-item v-if="row.empresas?.length" clickable v-close-popup @click="openBusiness360(row.empresas[0])"><q-item-section avatar><q-icon name="space_dashboard"/></q-item-section><q-item-section>Vista empresarial 360</q-item-section></q-item><q-item v-if="row.foto_path" clickable v-close-popup @click="verifyPhoto(row)"><q-item-section avatar><q-icon name="verified_user"/></q-item-section><q-item-section>Verificar foto</q-item-section></q-item><q-separator/><q-item clickable v-close-popup class="text-negative" @click="remove(row)"><q-item-section avatar><q-icon name="delete_forever"/></q-item-section><q-item-section>Eliminar</q-item-section></q-item></q-list></q-menu></q-btn>
      </q-card-section>
    </q-card>
