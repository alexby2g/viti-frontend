...
function remove(row){$q.dialog({title:'Eliminar cliente',message:`¿Eliminar a ${row.nombre}?`,cancel:true,persistent:true}).onOk(async()=>{try{await api.delete(`/clientes/${row.id}`);load()}catch(e){$q.notify({type:'negative',message:e.response?.data?.message||'No se puede eliminar.'})}})}
