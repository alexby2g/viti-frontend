...
<q-page class="viti-page">
  ...
  <q-table v-if="$q.screen.gt.sm" flat class="viti-table" :rows="rows" :columns="columns" row-key="id" :loading="loading" :pagination="{rowsPerPage:20,sortBy:'id',descending:true}">
    <template #top><q-input v-model="search" outlined dense placeholder="Buscar responsable, teléfono o empresa" style="min-width:320px"><template #prepend><q-icon name="search"/></template></q-input></template>
  ...
  <div v-else class="mobile-data-list">
    ...
    <q-input v-model="search" outlined dense placeholder="Buscar responsable, teléfono o empresa"><template #prepend><q-icon name="search"/></template></q-input>
