...
const auth=useAuthStore(),router=useRouter(),route=useRoute(),$q=useQuasar(),show=ref(false),showSecret=ref(false),mode=ref('admin')
...
<style scoped>.auth-page{min-height:100vh;background:radial-gradient(circle at 80% 20%,#d9ecff,transparent 34%),#f5f8fc}.login-card{width:min(500px,92vw);background:#fff;padding:42px;border-radius:22px;box-shadow:0 22px 65px rgba(18,45,78,.14)}.body--dark .auth-page{background:#06162b}.body--dark .login-card{background:#0c294c}</style>
