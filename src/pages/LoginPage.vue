...
const mode=ref(route.query.tipo==='admin'?'admin':'cliente')
      ...
      <section class="access-panel">
        ...
        <q-form ref="formRef" class="login-form" @submit="submit">
          <div data-error-field="acceso" class="field-block">
            <q-input v-model="form.acceso" outlined label="Usuario, teléfono o CI" hint="Puedes usar cualquiera de estos datos." :error="fieldErrors.has('acceso')" :error-message="fieldErrors.message('acceso')" :rules="[v=>!!v||'Ingresa tu usuario, teléfono o CI.']" @update:model-value="clearField('acceso')">
              <template #prepend><q-icon name="badge"/></template>
            </q-input>
          </div>
          <div data-error-field="password" class="field-block">
            <q-input v-model="form.password" outlined class="q-mt-md" :type="show?'text':'password'" label="Contraseña" :error="fieldErrors.has('password')" :error-message="fieldErrors.message('password')" :rules="[v=>!!v||'Ingresa tu contraseña.']" @update:model-value="clearField('password')">
              <template #append><q-icon :name="show?'visibility_off':'visibility'" class="cursor-pointer" @click="show=!show"/></template>
            </q-input>
          </div>
          <div v-if="mode==='admin'" data-error-field="codigo_secreto" class="admin-secret field-block">
            <q-input v-model="form.codigo_secreto" outlined class="q-mt-md" :type="showSecret?'text':'password'" label="Código secreto" hint="Solo se usa para cuentas de superadministrador." clearable :error="fieldErrors.has('codigo_secreto')" :error-message="fieldErrors.message('codigo_secreto')" @update:model-value="clearField('codigo_secreto')">
        ...
        <div class="under-action">
          ...
          <router-link v-if="mode==='cliente'" to="/registro">Crear mi cuenta</router-link>
...
<style scoped>
.auth-page{min-height:100vh;background:radial-gradient(circle at 18% 8%,rgba(11,117,147,.08),transparent 32%),linear-gradient(180deg,#f8fbfd 0%,#edf5f8 100%);color:#102a43}.access-header{min-height:74px;padding:0 clamp(18px,4vw,54px);display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #d7e3e8;background:#fff;position:sticky;top:0;z-index:20}.brand-link{display:flex;text-decoration:none;color:inherit}.header-actions{display:flex;align-items:center;gap:10px}.secure-note{display:inline-flex;align-items:center;gap:6px;font-size:12px;color:#425f72}.access-main{width:min(1080px,calc(100% - 32px));margin:0 auto;min-height:calc(100vh - 128px);display:grid;grid-template-columns:1fr 470px;gap:76px;align-items:center;padding:58px 0}.access-intro{max-width:620px}.eyebrow,.panel-kicker{font-size:11px;font-weight:900;letter-spacing:.15em;text-transform:uppercase;color:#0b7593}.access-intro h1{font-size:clamp(52px,6.6vw,88px);line-height:.94;letter-spacing:-.065em;margin:20px 0 24px;color:#102a43}.access-intro h1 span{color:#067e99}.access-intro p{font-size:18px;line-height:1.75;color:#3f5f72;max-width:590px}.intro-line{display:flex;align-items:center;gap:9px;margin-top:30px;flex-wrap:wrap;color:#3b596b;font-size:11px}.intro-line span{width:6px;height:6px;border-radius:50%;background:#12b76a}.access-panel{background:#fff;border:1px solid #c6d7df;border-radius:26px;padding:38px;box-shadow:0 24px 70px rgba(16,42,67,.12)}.access-panel h2{font-size:38px;line-height:1.02;letter-spacing:-.045em;margin:10px 0;color:#102a43}.panel-copy{margin:0 0 18px;color:#486576;line-height:1.6}.google-button{width:100%;height:52px;border:1px solid #b7cbd5;border-radius:12px;background:#fff;color:#162a37;font:inherit;font-weight:800;display:flex;align-items:center;justify-content:center;gap:11px;cursor:pointer;transition:.18s ease}.google-button:hover{border-color:#718e9f;box-shadow:0 8px 24px rgba(16,42,67,.09)}.google-mark{width:24px;height:24px;display:grid;place-items:center;color:#4285f4;font-weight:900;font-size:17px}.google-note{margin-top:10px;font-size:11px;line-height:1.5;color:#5d7787}.divider{display:flex;align-items:center;gap:12px;margin:23px 0;color:#536d7c;font-size:11px;font-weight:700}.divider:before,.divider:after{content:'';flex:1;height:1px;background:#cbd9e0}.mode-switch{display:grid;grid-template-columns:1fr 1fr;gap:4px;padding:4px;border-radius:13px;background:#e7eff3;margin-bottom:18px}.mode-switch button{border:0;background:transparent;border-radius:10px;padding:11px 8px;font:inherit;font-size:12px;font-weight:800;color:#456171;cursor:pointer}.mode-switch button.active{background:#146ef5;color:#fff;box-shadow:0 7px 18px rgba(20,110,245,.2)}.field-block :deep(.q-field__control){background:#fff;border-radius:12px;min-height:56px}.field-block :deep(.q-field--outlined .q-field__control:before){border:2px solid #a7bbc7}.field-block :deep(.q-field--outlined .q-field__control:hover:before){border-color:#607d8d}.field-block :deep(.q-field--focused .q-field__control:after){border:2px solid #146ef5;box-shadow:0 0 0 3px rgba(20,110,245,.1)}.field-block :deep(.q-field__label){color:#456171 !important;font-weight:700 !important}.field-block :deep(.q-field--float .q-field__label){color:#23485f !important}.field-block :deep(.q-field__native),.field-block :deep(input){color:#102a43 !important;font-size:16px !important;font-weight:600 !important}.field-block :deep(input::placeholder){color:#617b8b !important;opacity:1 !important}.field-block :deep(.q-field__bottom){color:#5c7483 !important}.field-block :deep(.q-icon){color:#506d7e !important}.field-block :deep(.q-field--error .q-field__label),.field-block :deep(.q-field--error .q-icon){color:#c62828 !important}.submit-button{height:54px;border-radius:12px}.under-action{text-align:center;margin-top:18px;font-size:12px;color:#587282}.under-action a{color:#146ef5;text-decoration:none;font-weight:800;margin-left:5px}.admin-secret{opacity:.96}.access-footer{text-align:center;padding:22px 16px;color:#607887;font-size:11px;border-top:1px solid #d7e3e8;background:#f5f9fb}.access-footer strong{color:#405868}@media(max-width:900px){.access-main{grid-template-columns:1fr;gap:34px;padding:42px 0}.access-intro{text-align:center;margin:0 auto}.access-intro p{margin-left:auto;margin-right:auto}.intro-line{justify-content:center}.access-panel{width:min(520px,100%);margin:0 auto}.access-intro h1{font-size:clamp(48px,13vw,72px)}}@media(max-width:600px){.access-header{min-height:64px;padding:0 14px}.secure-note{display:none}.header-actions .q-btn{padding-left:6px;padding-right:6px}.access-main{width:min(100% - 20px,1080px);padding:28px 0}.access-intro h1{font-size:50px}.access-intro p{font-size:16px}.access-panel{padding:26px 20px;border-radius:20px}.access-panel h2{font-size:31px}.auth-page{background:#f5f9fb}}
</style>
