...
const statusOptions=[
 {label:'Borrador',value:'borrador'},{label:'Pendiente de revisión',value:'en_revision'},{label:'Aprobada',value:'aprobada'},
...
const statusSummary=computed(()=>[
 {label:'Borradores',value:rows.value.filter(x=>x.estado==='borrador').length,icon:'edit_note'},
...
function stateIcon(value){return {borrador:'edit_note',en_revision:'schedule',aprobada:'check_circle',rechazada:'cancel',convertida:'account_tree',cerrada:'inventory_2'}[value]||'info'}
onMounted(load)
</script>
...
<q-page class="viti-page solicitudes-page">
  ...
  <q-table v-if="$q.screen.gt.sm" flat class="viti-table" :rows="filteredRows" :columns="columns" row-key="id" :loading="loading" :pagination="{rowsPerPage:20,sortBy:'id',descending:true}">
    ...
    <template #body-cell-estado="p"><q-td :props="p"><q-chip dense square class="status-chip" :color="stateColor(p.value)" text-color="white" :icon="stateIcon(p.value)">{{stateLabel(p.value)}}</q-chip></q-td></template>
