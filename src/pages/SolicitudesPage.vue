...
<q-page class="viti-page solicitudes-page">
  ...
  <q-table v-if="$q.screen.gt.sm" flat class="viti-table" :rows="filteredRows" :columns="columns" row-key="id" :loading="loading" :pagination="{rowsPerPage:20,sortBy:'id',descending:true}">
    ...
    <template #body-cell-titulo="p"><q-td :props="p"><button class="title-link" @click="router.push(`/solicitudes/${p.row.id}`)"><b>{{p.row.titulo}}</b><small>{{p.row.resumen||'Abrir solicitud'}}</small></button></q-td></template>
    <template #body-cell-cliente="p"><q-td :props="p"><div class="text-weight-bold">{{p.row.cliente?.nombre||'Sin contacto'}}</div><div class="text-caption text-grey-6">{{p.row.cliente?.telefono||''}}</div></q-td></template>
