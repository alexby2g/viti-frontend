...
<q-card v-if="item.aplicacion" flat class="viti-card q-mt-lg"><q-card-section><div class="section-label">Aplicación</div><div class="text-h6 text-weight-bold">{{item.aplicacion.nombre}}</div><div class="text-caption text-grey-6">{{item.aplicacion.entorno}} · {{item.aplicacion.estado}}</div><q-btn v-if="item.aplicacion.url" class="q-mt-md" color="primary" unelevated no-caps icon="open_in_new" label="Abrir aplicación" :href="item.aplicacion.url" target="_blank"/></q-card-section></q-card></div></div></template>
