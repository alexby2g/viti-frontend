...
const title=computed(()=>({inicio:'Inicio',agenda:'Agenda',ordenes:'Órdenes de servicio',clientes:'Clientes',equipos:'Equipos',tecnicos:'Técnicos',inventario:'Inventario técnico',pagos:'Pagos',garantias:'Garantías',historial:'Historial y reportes'}[section.value]||'Electrofrío'))
...
async function loadAll(){
  ...
  try{
    const requests=[
      ['resumen','inicio','resumen'],['clientes','clientes','clientes'],['equipos','equipos','equipos'],['tecnicos','tecnicos','tecnicos'],
      ['materiales','inventario','materiales'],['ordenes','ordenes','ordenes'],['pagos','pagos','pagos'],['garantias','garantias','garantias'],['historial','historial','historial'],
    ].filter(([,module])=>enabled(module))
    const responses=await Promise.all(requests.map(([, ,endpoint])=>api.get(`${base.value}/${endpoint}`)))
    const data=Object.fromEntries(requests.map(([key],index)=>[key,responses[index].data.data]))
    if(data.resumen)resumen.value=data.resumen
    if(data.clientes)clientes.value=data.clientes
    if(data.equipos)equipos.value=data.equipos
    if(data.tecnicos)tecnicos.value=data.tecnicos
    if(data.materiales)materiales.value=data.materiales
    if(data.ordenes)ordenes.value=data.ordenes
    if(data.pagos)pagos.value=data.pagos
    if(data.garantias)garantias.value=data.garantias
    if(data.historial)historial.value=data.historial
...
async function saveFinish(){try{const payload=clean({...finishForm});if(!enabled('garantias'))Object.assign(payload,{garantia_dias:0,condiciones_garantia:null});await api.post(`${base.value}/ordenes/${finishOrder.value.id}/finalizar`,payload);finishDialog.value=false;detailDialog.value=false;$q.notify({type:'positive',message:'Servicio finalizado y guardado en el historial.'});await loadAll()}catch(e){notifyError(e)}}
