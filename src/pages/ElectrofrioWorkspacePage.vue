...
<q-page class="electro-page q-pa-md q-pa-lg-xl">
  ...
  <template v-if="section==='inicio'">
    <div class="metrics-grid">
      ...
      <q-card flat class="app-card metric-card metric-card--accent"><q-card-section><div class="metric-icon"><q-icon name="payments" size="28px"/></div><div class="metric money-metric">{{money(resumen.por_cobrar)}}</div><div class="text-caption text-grey-6">Por cobrar</div></q-card-section></q-card>
      <q-card flat class="app-card metric-card metric-card--positive"><q-card-section><div class="metric-icon"><q-icon name="verified" size="28px"/></div><div class="metric">{{resumen.garantias_vigentes}}</div><div class="text-caption text-grey-6">Garantías vigentes</div></q-card-section></q-card>
      <q-card flat class="app-card metric-card" :class="resumen.stock_bajo?'metric-card--warning':'metric-card--positive'"><q-card-section><div class="metric-icon"><q-icon name="inventory_2" size="28px"/></div><div class="metric">{{resumen.stock_bajo}}</div><div class="text-caption text-grey-6">Materiales con stock bajo</div></q-card-section></q-card>
    </div>
...
:global(.body--dark) .app-card,:global(.body--dark) .app-table,:global(.body--dark) .mobile-card{box-shadow:0 12px 30px rgba(0,0,0,.2)}:global(.body--dark) .app-table :deep(.q-table thead th){color:#c7dae7}:global(.body--dark) .dialog-actions{background:var(--viti-card)}
