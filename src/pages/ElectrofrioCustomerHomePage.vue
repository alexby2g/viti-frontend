...
const loading=ref(true),error=ref(''),data=ref({cliente:null,sistema:null,ordenes:[]})
const openOrders=computed(()=>data.value.ordenes.filter(order=>order.etapa!=='cerrada'))
const heroStyle=computed(()=>({background:`linear-gradient(135deg, ${data.value.sistema?.color_primario||'#0B5F7A'}, ${data.value.sistema?.color_secundario||'#12B8C8'})`}))
...
<template>
  <q-page class="customer-page q-pa-md q-pb-xl">
    ...
    <template v-if="data.cliente">
      <div class="hero" :style="heroStyle"><div class="text-overline">{{data.sistema?.nombre_corto||'Mi servicio técnico'}}</div><div class="text-h4 text-weight-bold">Hola, {{data.cliente.nombre}}</div><div>Revisa tus próximas visitas y el avance de cada atención con {{data.cliente.empresa?.nombre_comercial||'la empresa de servicio'}}.</div><q-btn class="q-mt-md" color="white" text-color="primary" unelevated no-caps icon="forum" label="Escribir al negocio" to="/portal/electrofrio/mensajes"/></div>
