...
import { CHAT_FILE_ACCEPT, CHAT_FILE_MAX_BYTES, chatFileIcon, formatFileSize, isAllowedChatFile } from '../utils/chatFiles.js'
...
function onFile(e){const f=e.target?.files?.[0];if(!f)return;if(!isAllowedChatFile(f))return $q.notify({type:'negative',message:'Puedes adjuntar imágenes, PDF, Word, Excel o TXT.'});if(f.size>CHAT_FILE_MAX_BYTES)return $q.notify({type:'negative',message:'El archivo no puede superar 10 MB.'});attachment.value=f}
  ...
  <div class="row q-col-gutter-md inbox-wrap">
    ...
    <div v-show="!isMobile||chatOpen" class="col-12 col-md-8">
      <q-card v-if="current" flat class="viti-card messenger-card column no-wrap">
        ...
        <q-scroll-area ref="messagesBox" class="messages col" style="height:58vh"><div class="q-pa-md"><div v-for="m in current.mensajes||[]" :key="m.id" class="row q-mb-sm" :class="m.es_mio?'justify-end':'justify-start'"><div class="bubble" :class="m.es_mio?'bubble-mine':'bubble-team'"><div v-if="m.mensaje">{{m.mensaje}}</div><q-btn v-if="m.archivo_path" dense flat no-caps :icon="chatFileIcon(m)" :label="m.archivo_nombre||'Descargar archivo'" @click="download(m)"/><div v-if="m.archivo_tamano" class="text-caption">{{formatFileSize(m.archivo_tamano)}}</div><div class="text-caption message-meta q-mt-xs">{{m.usuario?.nombre}} · {{new Date(m.created_at).toLocaleString()}}</div></div></div></div></q-scroll-area>
        <q-separator/><q-card-section class="composer"><div v-if="attachment" class="attachment-selected q-mb-sm"><q-icon :name="chatFileIcon(attachment)" size="22px"/><div class="col min-width-0"><div class="text-caption text-weight-bold ellipsis">{{attachment.name}}</div><div class="text-caption text-grey-6">{{formatFileSize(attachment.size)}}</div></div><q-btn flat dense round icon="close" @click="attachment=null"/></div><div class="row items-end no-wrap q-gutter-sm"><q-btn flat round icon="attach_file" @click="chooseFile"><q-tooltip>Adjuntar imagen o documento</q-tooltip></q-btn><input ref="fileInput" hidden type="file" :accept="CHAT_FILE_ACCEPT" @change="onFile"/><q-input v-model="reply" autogrow outlined dense class="col" placeholder="Escribe una respuesta" @keydown.enter.exact.prevent="send"/><q-btn round color="primary" icon="send" :loading="sending" @click="send"/></div></q-card-section>
      </q-card>
...
<style scoped>
.inbox-wrap{min-height:70vh}.messenger-card{min-height:70vh}.min-width-0{min-width:0}.bubble{max-width:78%;padding:10px 13px;border-radius:16px;background:var(--viti-card);border:1px solid var(--viti-border)}.bubble-mine{background:var(--viti-primary,#1565c0);color:white;border-color:transparent}.messages{background:color-mix(in srgb,var(--viti-bg) 85%,var(--viti-card))}.attachment-selected{display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid var(--viti-border);border-radius:12px;background:var(--viti-surface-soft)}@media(max-width:600px){.viti-page{padding:0!important}.messenger-card,.contacts-card{border-radius:0!important;border-left:0;border-right:0}.bubble{max-width:88%}}
</style>
