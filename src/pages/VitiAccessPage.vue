...
function normalizeCode(codeForm) {
...
async function loadPlans() {
  ...
  try {
    ...
    if (selectedPlanCode.value && !selectedPlan.value) {
      selectedPlanCode.value = ''
      requestForm.plan_codigo = null
      requestForm.modalidad = null
      $q.notify({
        type: 'warning',
        message: 'El plan indicado en el enlace ya no está disponible. La solicitud continuará sin una preferencia de plan.',
      })
    }
  } catch {
    plans.value = []
      ...
      <q-card v-if="mode === 'code'" flat bordered class="access-card q-mt-lg">
        <q-card-section>
          ...
          <q-input
            ...
            :disable="loading"
            @update:model-value="normalizeCode(codeForm)"
            @keyup.enter="useCode"
          >
