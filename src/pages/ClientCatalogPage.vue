...
const $q=useQuasar();const tenant=useTenantStore();const loading=ref(true);const requesting=ref(null);const items=ref([])
...
async function requestApp(app){requesting.value=app.id;try{const {data}=await api.post(`/mi/catalogo/${app.id}/solicitar`);window.location.href=data.data.enlace_publico}catch(e){$q.notify({type:'negative',message:e.response?.data?.message||'No se pudo iniciar la solicitud.'})}finally{requesting.value=null}}
