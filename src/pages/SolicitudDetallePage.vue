...
const answeredSections=computed(()=>sections.value.map(section=>({...section,preguntas:(section.preguntas||[]).filter(question=>hasAnswer(question.id))})).filter(section=>section.preguntas.length))
