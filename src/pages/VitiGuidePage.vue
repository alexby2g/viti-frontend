...
const clientSteps = computed(() => [
  { key:'cuenta', icon:'person_add', title:'Crear tu cuenta', text:'Tu perfil identifica quién solicita el trabajo y guarda tus datos de contacto.', action:'Mi cuenta', to:'/mi-cuenta' },
  { key:'solicitud', icon:'assignment_add', title:'Crear una solicitud', text:'Cada nueva idea genera una solicitud independiente. Tus solicitudes anteriores se conservan.', action:'Mis solicitudes', to:'/mi-cuenta' },
  { key:'formulario', icon:'fact_check', title:'Completar el formulario', text:'Responde solo lo esencial: problema, funciones necesarias, usuarios, presupuesto y prioridades.', action:'Continuar', to:'/mi-cuenta' },
  { key:'revision', icon:'manage_search', title:'Revisión de AGR Studio', text:'AGR Studio revisa la viabilidad, organiza el alcance y puede contactarte por el buzón.', action:'Mi buzón', to:'/mi-buzon' },
  { key:'proyecto', icon:'account_tree', title:'Proyecto y desarrollo', text:'Cuando la solicitud se aprueba, nace un proyecto. Verás progreso, fechas, avances y archivos.', action:'Mi proyecto', to:'/mi-proyecto' },
  { key:'beta', icon:'science', title:'Beta y pruebas', text:'Cuando la versión de prueba esté lista, AGR Studio habilita el acceso para validar el sistema antes de la entrega.', action:'Aplicaciones', to:'/mi-aplicaciones' },
...
const adminSteps = [
  { key:'entrada', icon:'move_to_inbox', title:'Recibir solicitud', text:'Revisa datos del cliente, negocio, cuestionario y archivos recibidos.', action:'Solicitudes', to:'/solicitudes' },
  { key:'analisis', icon:'manage_search', title:'Evaluar viabilidad', text:'Define qué problema se resolverá, alcance, módulos, complejidad, tiempo y riesgos.', action:'Solicitudes', to:'/solicitudes' },
  ...
  { key:'desarrollo', icon:'terminal', title:'Desarrollar y publicar avances', text:'Actualiza progreso, módulos terminados, archivos y decisiones visibles para la empresa.', action:'Proyectos', to:'/proyectos' },
  { key:'beta', icon:'science', title:'Habilitar beta', text:'La beta se activa manualmente cuando esté lista. Los días de prueba no cambian por sí solos el estado técnico.', action:'Aplicaciones', to:'/aplicaciones' },
  ...
  { key:'entrega', icon:'verified_user', title:'Entregar y dar soporte', text:'Habilita el acceso de la empresa cuando corresponda y conserva soporte, mantenimiento e historial.', action:'Aplicaciones', to:'/aplicaciones' },
]
...
const visibleSteps = computed(() => mode.value === 'administrador' ? adminSteps : clientSteps.value)
...
function stateIcon(index, step) {
  const state = stateFor(index)
  return state === 'done' ? 'check' : step.icon
}
...
async function load() {
  ...
  try {
    if (isClient.value) {
      mode.value = 'cliente'
      const [reqResult, projectResult] = await Promise.allSettled([
  ...
  <q-page class="viti-page guide-page">
    ...
    <PageHeader eyebrow="AGR Studio · VITI" title="Guía de uso" subtitle="Un mapa claro del proceso para saber qué hacer, qué sigue y quién debe actuar.">
      <q-btn-toggle
        ...
        color="grey-9"
        text-color="grey-5"
        :options="[{label:'Administrador',value:'administrador',icon:'admin_panel_settings'},{label:'Empresa',value:'cliente',icon:'business'}]"
      />
    </PageHeader>
    ...
    <q-banner rounded class="guide-status q-mb-lg">
      ...
      <div class="text-body2 text-grey-6">
        {{ mode==='administrador'
          ? 'Este flujo es una guía operativa. Cada proyecto puede estar en una etapa distinta al mismo tiempo.'
          : 'El paso resaltado representa la etapa más cercana al estado actual de tu solicitud o proyecto.' }}
      </div>
    </q-banner>
        ...
        <q-card flat class="flow-card" :class="`flow-card--${stateFor(index)}`">
          <q-card-section>
            <div class="row items-start no-wrap q-gutter-md">
              <q-avatar :class="`step-avatar step-avatar--${stateFor(index)}`" size="48px">
                <q-icon :name="stateIcon(index,step)" size="25px" />
              </q-avatar>
              <div class="col min-width-0">
                <div class="text-overline step-number">Paso {{index+1}}</div>
                <div class="text-subtitle1 text-weight-bold">{{step.title}}</div>
                <div class="text-body2 text-grey-6 q-mt-xs">{{step.text}}</div>
              </div>
            </div>
            <q-btn v-if="step.to" flat dense no-caps color="primary" :label="step.action" icon-right="arrow_forward" class="q-mt-md" @click="go(step.to)" />
          </q-card-section>
        </q-card>
    ...
    <div class="row q-col-gutter-lg q-mt-lg">
      <div class="col-12 col-lg-7">
        <q-card flat class="viti-card">
          <q-card-section><div class="text-h6 text-weight-bold">Qué significa cada estado</div><div class="text-caption text-grey-6">Los estados no son adornos. Indican quién debe actuar y qué puede hacerse después.</div></q-card-section>
          <q-separator />
          <q-list separator>
            <q-item v-for="item in [
              ['Borrador','El formulario todavía se está completando.','edit_note','grey'],
              ['En revisión','AGR Studio está evaluando la solicitud.','manage_search','orange'],
              ['Aprobada','La solicitud es viable y puede convertirse en proyecto.','task_alt','positive'],
              ['Convertida','La solicitud ya tiene un proyecto asociado.','account_tree','purple'],
              ['Beta','Existe una versión de prueba, pero todavía no implica entrega.','science','indigo'],
              ['Finalizada','El desarrollo técnico terminó.','done_all','teal'],
              ['Entregada','La empresa ya tiene el acceso habilitado.','verified_user','positive'],
            ]" :key="item[0]">
              <q-item-section avatar><q-avatar :color="`${item[3]}-1`" :text-color="`${item[3]}-8`" :icon="item[2]"/></q-item-section>
              <q-item-section><q-item-label class="text-weight-bold">{{item[0]}}</q-item-label><q-item-label caption>{{item[1]}}</q-item-label></q-item-section>
            </q-item>
          </q-list>
        </q-card>
      </div>
      <div class="col-12 col-lg-5">
        <q-card flat class="viti-card full-height">
          <q-card-section><div class="text-h6 text-weight-bold">Ayuda rápida por módulo</div><div class="text-caption text-grey-6">Una referencia corta para no convertir cada pantalla en un manual de lavadora.</div></q-card-section>
          <q-separator />
          <q-list separator>
            <q-item><q-item-section avatar><q-icon name="assignment" color="primary"/></q-item-section><q-item-section><q-item-label>Solicitudes</q-item-label><q-item-label caption>Cada idea nueva debe tener su propia SOL y cuestionario.</q-item-label></q-item-section></q-item>
            <q-item><q-item-section avatar><q-icon name="account_tree" color="primary"/></q-item-section><q-item-section><q-item-label>Proyectos</q-item-label><q-item-label caption>Aquí vive el desarrollo, progreso, fechas y avances.</q-item-label></q-item-section></q-item>
            ...
            <q-item><q-item-section avatar><q-icon name="forum" color="primary"/></q-item-section><q-item-section><q-item-label>Mensajes</q-item-label><q-item-label caption>Canal para coordinación, imágenes, PDF y documentos permitidos.</q-item-label></q-item-section></q-item>
            <q-item><q-item-section avatar><q-icon name="apps" color="primary"/></q-item-section><q-item-section><q-item-label>Aplicaciones</q-item-label><q-item-label caption>Beta, producción, entrega y acceso son estados separados y controlados.</q-item-label></q-item-section></q-item>
          </q-list>
        </q-card>
      </div>
    </div>
