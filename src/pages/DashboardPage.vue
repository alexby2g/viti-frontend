    ...
    <q-card flat class="viti-card q-mt-lg agr-card">
      ...
      <q-card-section>
        <q-input v-model="agrMessage" outlined rounded dense placeholder="Escribe una orden para AGR..." @keyup.enter="askAgr()">
          ...
          <template #append><q-btn round flat dense icon="send" color="primary" :loading="agrLoading" @click="askAgr()" /></template>
        </q-input>

        <div class="row q-gutter-sm q-mt-md">
          <q-btn outline no-caps size="sm" label="Resumen" @click="useExample('resumen')" />
          <q-btn outline no-caps size="sm" label="Abrir clientes" @click="useExample('abrir clientes')" />
          <q-btn outline no-caps size="sm" label="Abrir solicitudes" @click="useExample('abrir solicitudes')" />
        ...
        <div class="agr-response">
          ...
          <div v-if="agrResponse.data?.clients !== undefined" class="row q-col-gutter-sm q-mt-md">
            <div v-for="item in [
              ...
              <q-card flat bordered class="summary-chip"><q-card-section class="text-center q-pa-sm"><div class="text-h6 text-weight-bold">{{ agrResponse.data[item.key] ?? 0 }}</div><div class="text-caption text-grey-6">{{ item.label }}</div></q-card-section></q-card>
            </div>
          </div>
          ...
          <q-list v-if="agrResponse.intent === 'search_client' && agrResponse.data?.results?.length" separator class="q-mt-md rounded-borders">
            <q-item v-for="client in agrResponse.data.results" :key="client.id">
              <q-item-section><q-item-label class="text-weight-bold">{{ client.nombre }}</q-item-label><q-item-label caption>{{ client.telefono || 'Sin teléfono' }} · {{ client.correo || 'Sin correo' }}</q-item-label></q-item-section>
          ...
          <q-list v-if="agrResponse.intent === 'search_company' && agrResponse.data?.results?.length" separator class="q-mt-md rounded-borders">
            <q-item v-for="company in agrResponse.data.results" :key="company.id">
              ...
              <q-item-section><q-item-label class="text-weight-bold">{{ company.nombre_comercial }}</q-item-label><q-item-label caption>{{ company.codigo || 'Sin código' }} · {{ company.ciudad || 'Sin ciudad' }} · {{ company.telefono || 'Sin teléfono' }}</q-item-label></q-item-section>
              <q-item-section side><q-badge :label="company.estado || 'sin estado'" /></q-item-section>
            </q-item>
          </q-list>

          <div v-if="agrResponse.data?.action?.type === 'navigate'" class="q-mt-md">
            <q-btn color="primary" unelevated no-caps icon="open_in_new" :label="agrResponse.data.action.label || 'Abrir'" @click="executeAgrAction" />
          </div>
          ...
          <div v-if="agrResponse.data?.commands?.length" class="q-mt-md">
            ...
            <div class="row q-gutter-xs"><q-chip v-for="command in agrResponse.data.commands" :key="command" dense>{{ command }}</q-chip></div>
          </div>
        </div>
      </q-card-section>
    </q-card>
    ...
    <div class="row q-col-gutter-lg q-mt-sm">
      <div class="col-12 col-lg-6">
        <q-card flat class="viti-card full-height">
          <q-card-section class="row items-center"><div><div class="text-h6 text-weight-bold">Necesitan atención</div><div class="text-caption text-grey-6">Entregas, vencimientos y suspensiones detectadas por VITI.</div></div><q-space /><q-btn flat dense no-caps color="primary" label="Centro SaaS" to="/saas" /></q-card-section>
          <q-separator />
      ...
      <div class="col-12 col-lg-6">
        <q-card flat class="viti-card"><q-card-section><div class="text-h6 text-weight-bold">Operación de hoy</div><div class="text-caption text-grey-6">Lo que todavía requiere trabajo humano, ese componente que insiste en seguir siendo necesario.</div></q-card-section><q-separator /><q-list>
