  ...
  <q-page class="viti-page">
    ...
    <q-card flat class="viti-card q-mt-lg agr-card">
      ...
      <q-card-section v-if="agrResponse" class="q-pt-none"><div class="agr-response"><div class="text-caption text-grey-6 q-mb-xs">AGR · {{ pretty(agrResponse.intent) }}</div><div class="text-body1">{{ agrResponse.message }}</div>
        ...
        <div v-if="agrResponse.data?.action?.type !== 'navigate' && agrResponse.data?.action" class="q-mt-md action-confirm"><div class="row items-center"><q-icon name="shield" color="orange-8" size="24px" /><div class="q-ml-sm"><div class="text-weight-bold">Acción protegida</div><div class="text-caption text-grey-7">AGR no modificará datos todavía. Primero preparará la operación y pedirá confirmación.</div></div></div><q-btn class="q-mt-md" outline color="orange-8" no-caps icon="fact_check" label="Preparar acción" @click="agrResponse = { ...agrResponse, data: { ...agrResponse.data, prepared: true } }" /></div>
        <div v-if="agrResponse.data?.prepared" class="q-mt-md prepared-action"><q-icon name="check_circle" color="positive" /><span class="q-ml-sm">Operación preparada. La siguiente fase conectará este paso con el formulario real y una confirmación explícita antes de guardar.</span></div>
...
.agr-card{overflow:hidden}.agr-response{border-radius:16px;background:rgba(25,118,210,.05);padding:16px}.summary-chip{height:100%}.action-confirm{border:1px solid rgba(245,158,11,.35);border-radius:14px;padding:14px;background:rgba(245,158,11,.06)}.prepared-action{display:flex;align-items:center;border-radius:14px;padding:12px;background:rgba(16,185,129,.08);color:#166534}
</style>
