...
const qrFileName=computed(()=>qrFile.value?.name||'')
const qrFileSize=computed(()=>{
  const bytes=Number(qrFile.value?.size||0)
  if(!bytes)return ''
...
function openQrPicker(){qrInput.value?.click()}
...
function clearQrSelection(){qrFile.value=null;onQrSelected(null)}
...
<q-page class="viti-page">
  ...
  <q-tab-panels v-model="tab" animated class="transparent">
    ...
    <q-tab-panel name="cobro" class="q-pa-none">
      <div class="row q-col-gutter-lg billing-cobro-grid"><div class="col-12 col-md-5"><q-card flat class="viti-card qr-card"><q-card-section><div class="text-h6 text-weight-bold">QR principal VITI</div><div class="text-caption text-grey-6">Este mismo QR se muestra a la empresa en su módulo de pagos.</div></q-card-section><q-card-section class="q-pt-none"><div class="qr-stage"><img :src="qrSrc" alt="QR de pago VITI" class="qr-img" @error="qrFailed=true"/></div><input ref="qrInput" class="qr-native-input" type="file" accept="image/png,image/jpeg,image/webp" @change="handleQrInput"/><div class="qr-upload-panel q-mt-md" :class="{'is-selected':Boolean(qrFile)}"><div class="qr-upload-main" role="button" tabindex="0" @click="openQrPicker" @keydown.enter.prevent="openQrPicker" @keydown.space.prevent="openQrPicker"><div class="qr-upload-icon"><q-icon :name="qrFile?'image':'upload_file'" size="24px"/></div><div class="qr-upload-copy"><div class="qr-upload-title">{{qrFile?'Nuevo QR seleccionado':'Cambiar código QR'}}</div><div class="qr-upload-caption">{{qrFile?qrFileName:'Selecciona una imagen desde tu equipo'}}</div><div class="qr-upload-meta">{{qrFile?qrFileSize:'PNG, JPG o WEBP · Máximo 5 MB'}}</div></div><q-btn outline no-caps color="primary" :label="qrFile?'Cambiar':'Seleccionar'" @click.stop="openQrPicker"/></div><div v-if="qrFile" class="qr-upload-actions"><q-btn flat no-caps color="grey-6" icon="close" label="Cancelar" @click="clearQrSelection"/><q-btn color="primary" unelevated no-caps icon="cloud_upload" label="Reemplazar QR" :loading="uploadingQr" @click="uploadQr"/></div></div></q-card-section></q-card></div><div class="col-12 col-md-7"><q-card flat class="viti-card billing-data-card"><q-card-section><div class="text-h6 text-weight-bold">Datos de cobro</div><div class="text-caption text-grey-6">Estos datos los verá la empresa junto al QR.</div></q-card-section><q-separator/><q-card-section class="billing-config-fields"><q-input v-model="configForm.banco" outlined label="Banco"/><q-input v-model="configForm.titular" outlined label="Titular"/><q-input v-model="configForm.observaciones" outlined type="textarea" label="Indicaciones opcionales" autogrow/></q-card-section><q-card-actions align="right" class="q-px-md q-pb-md"><q-btn color="primary" unelevated no-caps icon="save" label="Guardar datos" @click="saveConfig"/></q-card-actions></q-card></div></div>
    </q-tab-panel>
  </q-tab-panels>
...
<style scoped>
.billing-cobro-grid>.col-12{display:flex}.qr-card,.billing-data-card{width:100%;overflow:hidden}.qr-stage{display:flex;align-items:center;justify-content:center;min-height:250px;padding:20px;border:1px solid var(--viti-border);border-radius:16px;background:#fff}.qr-img{display:block;width:min(100%,300px);height:auto;max-height:320px;object-fit:contain;background:#fff}.qr-native-input{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}.qr-upload-panel{border:1px solid var(--viti-border);border-radius:14px;background:rgba(255,255,255,.02);overflow:hidden;transition:border-color .2s ease,background .2s ease}.qr-upload-panel:hover{border-color:rgba(255,145,39,.55);background:rgba(255,145,39,.035)}.qr-upload-panel.is-selected{border-color:rgba(255,145,39,.72);background:rgba(255,145,39,.055)}.qr-upload-main{display:flex;align-items:center;gap:13px;padding:14px;cursor:pointer;outline:none}.qr-upload-main:focus-visible{box-shadow:inset 0 0 0 2px var(--q-primary)}.qr-upload-icon{display:flex;align-items:center;justify-content:center;flex:0 0 42px;height:42px;border-radius:12px;background:rgba(255,145,39,.11);color:var(--q-primary)}.qr-upload-copy{min-width:0;flex:1;text-align:left}.qr-upload-title{font-size:14px;font-weight:750;color:var(--viti-text)}.qr-upload-caption{margin-top:2px;font-size:13px;color:var(--viti-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.qr-upload-meta{margin-top:4px;font-size:11px;color:var(--viti-muted)}.qr-upload-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;padding:10px 12px;border-top:1px solid var(--viti-border)}.billing-config-fields{display:grid;gap:16px}.empty-state{text-align:center;padding:36px;color:var(--viti-muted)}
...
@media(max-width:900px){.qr-stage{min-height:220px}.qr-img{width:min(100%,270px);max-height:290px}.proof-layout{grid-template-columns:1fr}.proof-preview{min-height:55vh;padding:14px}.proof-image{max-height:52vh}.proof-pdf{height:55vh}.proof-details{border-left:0;border-top:1px solid var(--viti-border);padding:20px}.proof-toolbar .q-toolbar__title{font-size:16px}.detail-grid{grid-template-columns:1fr 1fr}}
@media(max-width:600px){.qr-upload-main{align-items:flex-start;flex-wrap:wrap}.qr-upload-main>.q-btn{width:100%;margin-top:2px}.qr-upload-actions{flex-direction:column-reverse;align-items:stretch}.qr-upload-actions>.q-btn{width:100%}.proof-toolbar{padding:0 6px}.proof-toolbar .q-btn .q-btn__content .block{display:none}.proof-preview{min-height:48vh}.proof-pdf{height:48vh}.detail-grid{grid-template-columns:1fr}}
</style>
