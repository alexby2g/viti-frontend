...
function toggleModule(key) {
  ...
  config.modulos = enabled(key) ? config.modulos.filter(x => x !== key) : [...config.modulos, key]
}
...
async function save() {
  ...
  try {
    ...
    await api.put(`/aplicaciones/${applicationId}`, payload)
...
<template>
  <q-page class="viti-page q-pa-md">
    ...
    <q-inner-loading :showing="loading"><q-spinner-gears color="primary" size="48px" /></q-inner-loading>
      ...
      <div class="col-12 col-lg-7">
        <q-card flat class="viti-card">
          <q-card-section>
            ...
            <q-banner rounded class="bg-orange-1 text-orange-10 q-mt-lg">
              No se puede habilitar desde aquí un módulo que el plan de la empresa no permita.
            </q-banner>
          </q-card-section>
        </q-card>
      </div>
