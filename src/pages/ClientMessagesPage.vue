...
<template>
  <q-page class="viti-page client-chat-page">
    <PageHeader v-if="!isMobile || !mobileChatOpen" eyebrow="Atención" title="Mi buzón" subtitle="Mensajes y atención con VITI." />
      ...
      <q-card v-if="current" flat class="viti-card inbox-item" @click="openConversation">
        <q-card-section class="row items-center no-wrap">
          ...
          <div class="col min-width-0">
            ...
            <div class="text-caption text-grey-6 ellipsis">{{ lastMessage?.mensaje || (lastMessage?.archivo_url ? '📷 Imagen adjunta' : 'Sin mensajes') }}</div>
            <div class="text-caption text-grey-5">{{ lastMessage?.created_at ? formatDateTime(lastMessage.created_at) : 'Sin mensajes todavía' }}</div>
          </div>
    ...
    <div v-if="showConversation" class="conversation-shell">
      ...
      <q-card v-if="current" flat class="viti-card messenger-card">
        <q-card-section class="row items-center no-wrap chat-header">
          ...
          <div class="col min-width-0"><div class="text-subtitle1 text-weight-bold ellipsis">{{ responsibleName }}</div><div class="text-caption text-grey-6 ellipsis">Atención VITI</div></div>
