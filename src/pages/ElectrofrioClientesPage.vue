...
const clientSummary = computed(() => ({
  equipos:clientEquipment.value.length,
  citas:activeClientOrders.value.length,
  servicios:clientOrders.value.length,
}))
...
async function save(){
  ...
  try{
    ...
    $q.notify({type:'positive',message:wasNew?'Cliente registrado. Ahora puedes agregar su equipo y agendar la visita.':'Cliente actualizado.'})
...
function remove(row){
  $q.dialog({title:'Eliminar cliente',message:`¿Eliminar a ${row.nombre}? Si tiene equipos u órdenes, el sistema protegerá su historial.`,cancel:true,persistent:true}).onOk(async()=>{
...
async function loadClientContext(){
  ...
  }catch(error){notifyError(error,'No se pudo cargar la ficha completa del cliente.')}
  finally{detailLoading.value=false}
}

function openEquipment(row=null, schedule=false){
  if(!selected.value)return
  editingEquipmentId.value=row?.id||null
  scheduleAfterEquipmentSave.value=Boolean(schedule)
...
async function saveEquipment(forceSchedule=false){
  if(!selected.value||!String(equipmentForm.tipo||'').trim()){
    $q.notify({type:'warning',message:'Indica el tipo de equipo.'})
    return
  }
  saving.value=true
  try{
    const payload=clean({...equipmentForm,cliente_id:selected.value.id})
    const response=editingEquipmentId.value
      ? await api.put(`${base.value}/equipos/${editingEquipmentId.value}`,payload)
      : await api.post(`${base.value}/equipos`,payload)
    ...
    if(shouldSchedule&&equipmentId>0)await scheduleAppointment(equipmentId)
  }catch(error){notifyError(error,'No se pudo guardar el equipo.')}
  finally{saving.value=false;scheduleAfterEquipmentSave.value=false}
}

async function scheduleAppointment(equipmentId=null){
  if(!selected.value||!hasModule('ordenes'))return
  if(!equipmentId&&hasModule('equipos')&&clientEquipment.value.length===0){
    openEquipment(null,true)
    return
  }
  const selectedEquipment=equipmentId || (clientEquipment.value.length===1 ? clientEquipment.value[0].id : null)
  detailDialog.value=false
  await router.push({
    path:`${appBase.value}/ordenes`,
    query:{nuevo:'1',cliente:String(selected.value.id),...(selectedEquipment?{equipo:String(selectedEquipment)}:{})},
  })
}
function openService(row){
  detailDialog.value=false
  router.push({path:`${appBase.value}/ordenes`,query:{servicio:String(row.id)}})
}
  ...
  <q-page padding class="clients-page">
    <div class="row items-start justify-between q-col-gutter-md q-mb-lg">
      <div class="col-12 col-md">
        ...
        <h1 class="text-h4 text-weight-bold q-my-xs">Clientes y atención</h1>
        <div class="text-body2 text-grey-7">Cada cliente concentra sus datos, equipos y citas. Entra a su ficha para agregar otro equipo o programar una nueva visita sin volver a registrar la misma información.</div>
      </div>
    ...
    <q-banner rounded class="flow-banner q-mb-lg">
      <template #avatar><q-icon name="account_tree" color="primary"/></template>
      <strong>Flujo rápido:</strong> registra al cliente una sola vez → agrega uno o varios equipos → agenda las visitas desde esa misma ficha.
    </q-banner>
    ...
    <q-card flat bordered>
      ...
      <q-table flat :rows="rows" :columns="columns" row-key="id" :loading="loading" :grid="$q.screen.lt.md" :pagination="{rowsPerPage:20}" no-data-label="Aún no hay clientes registrados.">
        ...
        <template #body-cell-acciones="props"><q-td :props="props">
          <q-btn flat round dense color="primary" icon="folder_shared" @click="openClient(props.row)"><q-tooltip>Abrir ficha: equipos y citas</q-tooltip></q-btn>
    ...
    <q-dialog v-model="detailDialog" :maximized="$q.screen.lt.md">
      <q-card v-if="selected" class="client-detail-card">
        <q-card-section class="client-detail-header">
          <div class="row items-start q-col-gutter-md">
            <div class="col-auto"><q-avatar size="52px" color="primary" text-color="white" icon="person"/></div>
            <div class="col"><div class="text-overline">Ficha del cliente</div><div class="text-h5 text-weight-bold">{{selected.nombre}}</div><div class="text-caption">{{selected.telefono||'Sin teléfono'}} · {{selected.direccion||'Sin dirección registrada'}}</div></div>
            <div class="col-auto row q-gutter-xs"><q-btn flat round color="white" icon="edit" @click="openForm(selected)"><q-tooltip>Editar cliente</q-tooltip></q-btn><q-btn flat round color="white" icon="close" v-close-popup/></div>
          </div>
        </q-card-section>
        ...
        <q-card-section class="q-pa-lg">
          <div class="row q-col-gutter-sm q-mb-lg">
            <div class="col-4"><q-card flat bordered class="summary-card"><q-card-section><div class="text-h5 text-weight-bold">{{clientSummary.equipos}}</div><div class="text-caption text-grey-7">Equipos</div></q-card-section></q-card></div>
            <div class="col-4"><q-card flat bordered class="summary-card"><q-card-section><div class="text-h5 text-weight-bold">{{clientSummary.citas}}</div><div class="text-caption text-grey-7">Atenciones abiertas</div></q-card-section></q-card></div>
            <div class="col-4"><q-card flat bordered class="summary-card"><q-card-section><div class="text-h5 text-weight-bold">{{clientSummary.servicios}}</div><div class="text-caption text-grey-7">Servicios totales</div></q-card-section></q-card></div>
          </div>

          <div class="row q-col-gutter-sm q-mb-xl">
            <div v-if="hasModule('equipos')" class="col-12 col-sm-auto"><q-btn color="primary" icon="add_circle" label="Agregar equipo" no-caps @click="openEquipment()"/></div>
            <div v-if="hasModule('ordenes')" class="col-12 col-sm-auto"><q-btn outline color="primary" icon="event_available" label="Agendar cita" no-caps @click="scheduleAppointment()"/></div>
          </div>

          <section v-if="hasModule('equipos')" class="detail-section q-mb-xl">
            <div class="row items-center q-mb-md"><div><div class="text-h6 text-weight-bold">Equipos del cliente</div><div class="text-caption text-grey-7">Agrega todos los equipos que pertenezcan al mismo cliente y agenda cada visita sobre el equipo correcto.</div></div><q-space/><q-btn flat color="primary" icon="add" label="Otro equipo" no-caps @click="openEquipment()"/></div>
            <div v-if="clientEquipment.length" class="equipment-grid">
              <q-card v-for="item in clientEquipment" :key="item.id" flat bordered class="equipment-card">
                <q-card-section><div class="row items-start no-wrap"><q-avatar color="blue-1" text-color="primary" icon="ac_unit"/><div class="col q-ml-md"><div class="text-weight-bold">{{equipmentLabel(item)}}</div><div class="text-caption text-grey-7">{{item.serie?`Serie ${item.serie}`:'Sin serie'}} · {{item.capacidad||'Capacidad sin registrar'}}</div><div class="text-caption q-mt-xs">{{item.ubicacion||'Ubicación sin registrar'}}</div></div><q-badge :color="item.activo?'positive':'grey'" :label="item.activo?'Activo':'Inactivo'"/></div></q-card-section>
                <q-separator/>
                <q-card-actions align="right"><q-btn flat icon="edit" label="Editar" no-caps @click="openEquipment(item)"/><q-btn v-if="hasModule('ordenes')&&item.activo" flat color="primary" icon="event_available" label="Agendar cita" no-caps @click="scheduleAppointment(item.id)"/></q-card-actions>
              </q-card>
            </div>
          ...
          <section v-if="hasModule('ordenes')" class="detail-section">
            <div class="row items-center q-mb-md"><div><div class="text-h6 text-weight-bold">Citas y servicios</div><div class="text-caption text-grey-7">Historial de atenciones del mismo cliente, con el equipo correspondiente.</div></div><q-space/><q-btn flat color="primary" icon="event_available" label="Nueva cita" no-caps @click="scheduleAppointment()"/></div>
            <q-list v-if="orderedClientOrders.length" bordered separator class="rounded-borders">
              <q-item v-for="order in orderedClientOrders.slice(0,20)" :key="order.id" clickable @click="openService(order)">
                <q-item-section avatar><q-avatar :color="stateColor(order)" text-color="white" icon="event"/></q-item-section>
                <q-item-section><q-item-label class="text-weight-bold">{{order.codigo}} · {{equipmentLabel(order)}}</q-item-label><q-item-label caption>{{date(order.fecha_cita)}} {{time(order.hora_cita)}} · {{order.tipo_servicio||'Servicio técnico'}} · {{order.problema_reportado}}</q-item-label></q-item-section>
                <q-item-section side><q-badge :color="stateColor(order)" :label="pretty(currentState(order))"/><q-icon name="chevron_right" class="q-mt-xs"/></q-item-section>
              </q-item>
            </q-list>
    ...
    <q-dialog v-model="equipmentDialog" persistent>
      <q-card class="equipment-dialog">
        <q-card-section class="row items-start"><div><div class="text-overline text-primary">{{selected?.nombre}}</div><div class="text-h5 text-weight-bold">{{editingEquipmentId?'Editar equipo':'Registrar equipo'}}</div><div class="text-caption text-grey-7">El equipo quedará vinculado automáticamente a este cliente.</div></div><q-space/><q-btn flat round icon="close" v-close-popup/></q-card-section>
        <q-separator/>
        <q-card-section class="row q-col-gutter-md q-gutter-y-md">
          <div class="col-12 col-sm-6"><q-select v-model="equipmentForm.tipo" outlined use-input fill-input hide-selected new-value-mode="add-unique" :options="equipmentTypes" label="Tipo de equipo *"/></div>
          <div class="col-12 col-sm-6"><q-input v-model="equipmentForm.marca" outlined label="Marca"/></div>
          <div class="col-12 col-sm-6"><q-input v-model="equipmentForm.modelo" outlined label="Modelo"/></div>
          <div class="col-12 col-sm-6"><q-input v-model="equipmentForm.serie" outlined label="Serie"/></div>
          <div class="col-12 col-sm-6"><q-input v-model="equipmentForm.capacidad" outlined label="Capacidad"/></div>
          <div class="col-12 col-sm-6"><q-input v-model="equipmentForm.ubicacion" outlined label="Ubicación dentro del inmueble"/></div>
          <div class="col-12"><q-input v-model="equipmentForm.observaciones" type="textarea" autogrow outlined label="Observaciones"/></div>
          <div class="col-12"><q-toggle v-model="equipmentForm.activo" label="Equipo activo"/></div>
        </q-card-section>
        <q-separator/>
        <q-card-actions align="right" class="q-pa-md"><q-btn flat label="Cancelar" no-caps v-close-popup/><q-btn v-if="!editingEquipmentId&&hasModule('ordenes')" outline color="primary" icon="event_available" label="Guardar y agendar" no-caps :loading="saving" @click="saveEquipment(true)"/><q-btn color="primary" icon="save" :label="editingEquipmentId?'Guardar cambios':'Guardar equipo'" no-caps :loading="saving" @click="saveEquipment(false)"/></q-card-actions>
      </q-card>
    </q-dialog>
