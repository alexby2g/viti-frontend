...
const estadoLabel = computed(() => String(empresa.value?.estado || 'sin estado').replaceAll('_', ' '))

const operationalHealth = computed(() => {
  if (!empresa.value) return { label: 'Sin datos', color: 'grey-7', icon: 'help_outline' }
  if (empresa.value.estado === 'inactivo') return { label: 'Inactiva', color: 'negative', icon: 'pause_circle' }
  if (counts.value.solicitudes > 0 && counts.value.proyectos === 0) return { label: 'Esperando ejecución', color: 'warning', icon: 'pending_actions' }
  if (counts.value.proyectos > 0 && counts.value.aplicaciones === 0) return { label: 'En construcción', color: 'deep-purple', icon: 'build_circle' }
  return { label: 'Operación activa', color: 'positive', icon: 'check_circle' }
})
...
const activity = computed(() => {
  ...
  return events.filter(item => item.date).sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 8)
})
    ...
    <template v-if="empresa && !loading">
      ...
      <div class="row q-col-gutter-lg">
          ...
          <q-card flat class="viti-card full-height">
            ...
            <q-list v-if="empresa.usuarios?.length" separator>
              <q-item v-for="user in empresa.usuarios" :key="user.id">
                ...
                <q-item-section><q-item-label class="text-weight-medium">{{ [user.nombre, user.apellido].filter(Boolean).join(' ') || user.usuario }}</q-item-label><q-item-label caption>@{{ user.usuario }} · {{ user.rol }}</q-item-label></q-item-section>
        ...
        <div class="col-12 col-lg-5">
          <q-card flat class="viti-card full-height">
            <q-card-section><div class="text-subtitle1 text-weight-bold">Perfil empresarial</div><div class="text-caption text-grey-6">Datos que ayudan a VITI a entender la cuenta.</div></q-card-section>
            <q-separator />
            <q-card-section><div class="profile-grid">
        ...
        <div class="col-12 col-lg-7">
          <q-card flat class="viti-card full-height">
            <q-card-section class="row items-center"><div><div class="text-subtitle1 text-weight-bold">Actividad reciente</div><div class="text-caption text-grey-6">Una lectura rápida del movimiento de la cuenta.</div></div><q-space /><q-chip dense outline color="grey-7" icon="history">{{ activity.length }} visibles</q-chip></q-card-section>
            <q-separator />
        ...
        <div class="col-12 col-lg-5">
          <q-card flat class="viti-card full-height">
            <q-card-section><div class="text-subtitle1 text-weight-bold">AGR para esta cuenta</div><div class="text-caption text-grey-6">Lectura contextual sin inventar datos ni acciones.</div></q-card-section>
            <q-separator />
            <q-card-section>
              <div class="agr-signal"><q-icon name="smart_toy" color="primary" size="26px" /><div><div class="text-weight-bold">AGR listo para analizar</div><div class="text-caption text-grey-6">La cuenta ya expone entidades conectadas para el análisis operativo general.</div></div></div>
              <div class="q-mt-md q-gutter-sm"><q-chip dense outline color="primary">{{ counts.solicitudes }} solicitudes</q-chip><q-chip dense outline color="deep-purple">{{ counts.proyectos }} proyectos</q-chip><q-chip dense outline color="teal">{{ counts.aplicaciones }} apps</q-chip></div>
        ...
        <div class="col-12">
          <q-card flat class="viti-card">
            <q-card-section class="row items-center"><div><div class="text-subtitle1 text-weight-bold">Solicitudes y proyectos</div><div class="text-caption text-grey-6">Los registros principales relacionados con la cuenta.</div></div><q-space /><div class="row q-gutter-xs"><q-btn flat dense no-caps color="primary" label="Solicitudes" to="/solicitudes"/><q-btn flat dense no-caps color="primary" label="Proyectos" to="/proyectos"/></div></q-card-section>
            <q-separator />
            <div class="row q-col-gutter-lg q-pa-md">
              <div class="col-12 col-lg-6">
                <q-list v-if="empresa.solicitudes?.length" separator>
                  <q-item v-for="item in empresa.solicitudes.slice(0,5)" :key="item.id"><q-item-section avatar><q-icon name="assignment" color="primary" /></q-item-section><q-item-section><q-item-label class="text-weight-medium">{{ item.titulo || item.nombre || `Solicitud #${item.id}` }}</q-item-label><q-item-label caption>{{ item.estado || 'sin estado' }}</q-item-label></q-item-section><q-item-section side><q-badge outline color="grey-7" :label="formatDateTime(item.created_at)" /></q-item-section></q-item>
                </q-list>
              ...
              <div class="col-12 col-lg-6">
                <q-list v-if="empresa.proyectos?.length || empresa.aplicaciones?.length" separator>
                  <q-item v-for="item in (empresa.proyectos || []).slice(0,4)" :key="`p-${item.id}`"><q-item-section avatar><q-icon name="account_tree" color="deep-purple" /></q-item-section><q-item-section><q-item-label class="text-weight-medium">{{ item.nombre || item.codigo || `Proyecto #${item.id}` }}</q-item-label><q-item-label caption>{{ item.estado || 'sin estado' }} · {{ item.progreso ?? 0 }}%</q-item-label></q-item-section></q-item>
                  <q-item v-for="item in (empresa.aplicaciones || []).slice(0,4)" :key="`a-${item.id}`"><q-item-section avatar><q-icon name="apps" color="teal" /></q-item-section><q-item-section><q-item-label class="text-weight-medium">{{ item.nombre || `Aplicación #${item.id}` }}</q-item-label><q-item-label caption>{{ item.estado || 'sin estado' }}</q-item-label></q-item-section></q-item>
                </q-list>
      ...
      <q-card v-if="empresa.observaciones" flat class="viti-card q-mt-lg"><q-card-section><div class="text-subtitle1 text-weight-bold">Observaciones</div><div class="q-mt-sm text-body2">{{ empresa.observaciones }}</div></q-card-section></q-card>
    </template>
  </q-page>
</template>
