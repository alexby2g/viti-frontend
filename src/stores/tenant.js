...
export const useTenantStore = defineStore('tenant', {
  ...
  getters: {
    active(state) { return state.businesses.find(item => Number(item.id) === Number(state.activeId)) || null },
    needsSelection(state) { return state.businesses.length > 1 && !state.activeId },
  ...
  actions: {
    async load() {
      ...
      const savedId = Number(localStorage.getItem('viti-empresa-id') || 0)
      const savedIsValid = this.businesses.some(item => Number(item.id) === savedId)
      const nextId = savedIsValid ? savedId : (this.businesses.length === 1 ? Number(this.businesses[0]?.id || 0) : 0)
    ...
    select(id) {
      ...
      if (nextId && !this.businesses.some(item => Number(item.id) === nextId)) return false
